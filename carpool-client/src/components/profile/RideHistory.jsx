  ...
  const getRideHistory = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/v1/rides/getridehistory",
        {
  ...
  if(rides.length === 0){
    return(
      <div className="w-[100%] pt-10 min-h-screen bg-gradient-to-r from-neutral-softPink to-neutral-lightLavender">
        <h1 className="w-fit p-5 bg-white/60 backdrop-blur-md border border-transparent rounded-2xl shadow-xl text-secondary-purpleDark text-2xl font-bold mx-auto mb-10">NO RIDES FOUND!</h1>
    ...
    {rides?.map((item) => {
          ...
          "
              ...
              <div className="h-20 flex items-center justify-around">
                ...
                <div className="flex items-center font-bold">
                  <GrUserManager />
                  <p className="ml-2">{item.rideId.owner.name}</p>
                </div>

                <button className="bg-primary-yellow p-2 rounded-lg font-bold hover:bg-secondary-orange hover:text-contrast-white">
                  Completed
                </button>
              </div>
            </div>
