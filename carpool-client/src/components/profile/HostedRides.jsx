  ...
  const getRideHistory = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/v1/rides/gethostedrides",
        {
      ...
      if (response.data.success) {
        setRides(response.data.findRide);
        setLoading(false);
      }
    } catch (error) {
      setLoading(false);
  ...
  if(rides.length === 0){
    return(
      <div className="w-[100%] pt-10 min-h-screen bg-gradient-to-r from-neutral-softPink to-neutral-lightLavender">
        <h1 className="w-fit p-5 bg-white/60 backdrop-blur-md border border-transparent rounded-2xl shadow-xl text-secondary-purpleDark text-2xl font-bold mx-auto mb-10">NO HOSTED RIDES FOUND!</h1>
        <div className="flex items-center justify-center">
        <Link to='/addride'>
        <button  className='w-60 bg-primary-yellow text-beige-light py-2 px-4 rounded-lg hover:bg-secondary-orange hover:text-contrast-white font-semibold mx-auto'>Add Ride</button>
        </Link>
        </div>
         </div>
    )
          ...
          "
              ...
              <div className="h-20 flex items-center justify-around">
                <div className="flex items-center font-bold">
                  <SlCalender />
                  <p className="ml-2">{item.date.slice(0,10)}</p>
                </div>
